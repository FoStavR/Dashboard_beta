...
def show_inbound_dashboard(df):
    ...
    col2.metric("FDC", df["FDC"].nunique())
    ...
    col6.metric("Total Pallets", int(df['Pallets'].sum()))
    col7.metric("Total SKU", int(df['Sku Qty.'].sum()))
    col8.metric("Total Boxes",int(df['Boxes'].sum()))
    col9.metric("Total Reels",int(df['Reels'].sum()))
    col10.metric("Total CBM", round(df['CBM'].sum(),2))
    ...
    with cl2:
        if 'FDC' in df.columns:
            st.subheader("Total Shipments per FDC 🏬🔎")
    ...
    with tg1:
        if 'CUSTOMS FORMALITIES' in df.columns:
            ...
            fig = px.bar(
                ...
                labels={'Formality':'Custom Formality', 'Count':'Number of Occurrences'},
                title="Custom Formalities Distribution"
            )
    ...
    with tg2: 
       if 'Description' in df.columns:
            ...
            if view_option == "Table 📋":
                st.subheader("Top Goods Table 🏆🔎")
                st.dataframe(goods_counts)

            elif view_option == "Pie Chart 🥧":
                st.subheader("Top Goods Pie Chart 🏆🔎")
    ...
    if 'Vessel/Voyage' in df.columns:
        ...
        st.subheader("Top 15 Vessels (Inbound) 🚢🔎")
...
def show_outbound_dashboard(df):
    ...
    with dest2:
        ...
        if 'Description' in df.columns:
            ...
            st.subheader("Top Descriptions (Outbound)🏆🔎")
            ...
            if view_mode == "Chart":
                ...
                st.dataframe(desc_counts, use_container_width=True)
    ...
    with col1:
        ...
        if 'CUSTOMS FORMALITIES' in df.columns:
            st.subheader("Custom Formalities Breakdown 📑🔎")
            st.bar_chart(df['CUSTOMS FORMALITIES'].value_counts()
                         ,height=300)
    with col2:
        # ==============================
        # 📦 CONTAINER SIZE
        ...
        if 'Container Size/type' in df.columns:
            ...
            # Count occurrences of 20' and 40'
            ...
            container_df = pd.Series(container_counts)

            st.bar_chart(container_df)
    with col3:
        # ==============================
        # 📦 GOODS TYPE
        # ==============================
        if 'Goods Type' in df.columns:
            ...
            st.bar_chart(df['Goods Type'].value_counts(),height=365)
    with col4:
        # ==============================
        # 🚚 SHIPPING MODE
        # ==============================
        if 'Shipping MODE' in df.columns:
            ...
            st.bar_chart(shipping_mode.value_counts(), height=377)
    
    if 'Vessel/Voyage' in df.columns:

        st.subheader("Top 15 Vessels (Outbound) 🚢🔎")
